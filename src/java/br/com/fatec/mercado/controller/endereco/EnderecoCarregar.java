...
public class EnderecoCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            GenericDAO oEnderecoDAO = new EnderecoDAO();
            request.setAttribute("endereco", oEnderecoDAO.carregar(idEndereco));
            
            //dispacha objeto de lombada para a pagina jsp
            request.getRequestDispatcher("/cadastros/endereco/enderecoCadastrar.jsp")
                    .forward(request, response);
        }
        catch(Exception ex){
            System.out.println("Problemas no Servlet ao Nova Endereco! "
                    + "Erro: " + ex.getMessage());
        }
